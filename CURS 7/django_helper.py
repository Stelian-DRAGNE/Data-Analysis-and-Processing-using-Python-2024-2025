...
# Verifica sistemul de operare (Windows)
if os.name == 'nt':
...
def _add_app_to_intslled_apps(app_name = 'test_app', project_name = 'test_project'):
    # Current working directory
    cwd = os.getcwd()
    print("Inainte de a schimba path-ul.", cwd)
    ## Schimba path-ul
    ...
    has_encounter_istalled_apps = False
    for index, line in enumerate(settings_content):
        if "INSTALLED_APPS = [" in line:
            has_encounter_istalled_apps = True
        elif has_encounter_istalled_apps and ("]" in line):
