...
def create_view_url_and_template(view_name, url_name, template_name, app_name, project_name):
    # Schimbăm directorul de lucru la proiect înainte de a crea aplicația
    cwd = os.getcwd()
    proj_path = project_path(project_name)
    os.chdir(proj_path)
    ...
    create_url = (url_name, view_name, app_name)
    ...
    while True:
        ...
        while True:
            ...
            while True:
                ...
                if view_name == "":
                    break
                url_name = input(f" Introdu numele PATH-ului pt {view_name}: ")
                template_name = input(f" Introdu numele template-ului pt view-ul {view_name}: ")
                view_templates_per_application.append((view_name, url_name, template_name, APP_NAME))
        ...
        for view, template, app in view_templates_per_application:
            create_view_url_and_template(view, template, app, PROJECT_NAME)
